...
<div class="container">
    ...
    <table>
        ...
        <%
            ...
            List<RecipeWithServiceId> recipes = (List<RecipeWithServiceId>) pageData.get("recipes");
            ...
            if (recipes != null && !recipes.isEmpty()) {
                for (RecipeWithServiceId recipe : recipes) {
        %>
        <tr>
            <td class="title"><a href="<%=request.getContextPath()%>/recipes/<%=recipe.getId()%>"><%= recipe.getFoodName() %></a></td>
            <td><%= recipe.getWriter() %></td>
            <td class="created-at"><%= recipe.getCreatedAt() %></td>
    ...
    <div class="pagination">
        <%
            if(startPage != 1) {
        %>
        <a href="<%=request.getContextPath()%>/recipes?page=<%=startPage - 1%>">&lt;이전</a>
        <%
            }
            for (int i = startPage; i <= endPage; i++) {
                if (i == currentPage) {
        %>
        <a class="current-page" href="<%=request.getContextPath()%>/recipes?page=<%=i%>"><%=i%></a>
        <%
        } else {
        %>
        <a href="<%=request.getContextPath()%>/recipes?page=<%=i%>"><%=i%></a>
        <%
                }
            }
            if(endPage != totalPageCount) {
        %>
        <a href="<%=request.getContextPath()%>/recipes?page=<%=endPage + 1%>">다음&gt;</a>
        <%
            }
        %>
    </div>
